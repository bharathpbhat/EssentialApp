...
//  EssentialApp
//
//  Created by Bharath Bhat on 2/4/23.
//
...
struct CapturedFrame {
    static let invalid = CapturedFrame(pixelBuffer: nil, surface: nil, contentRect: .zero, contentScale: 0, scaleFactor: 0,  displayTime: 0, dirtyRects: [])
    ...
    let dirtyRects: Array<CGRect?>
    
    var dirtyRectsTotalArea: Double {
        var areasSum = 0.0
        for maybeCGRect in dirtyRects {
            if let rect = maybeCGRect {
                areasSum += rect.height * rect.width
            }
        }
        return areasSum
    }
...
private class CaptureEngineStreamOutput: NSObject, SCStreamOutput, SCStreamDelegate {
    ...
    private func createFrame(for sampleBuffer: CMSampleBuffer) -> CapturedFrame? {
        ...
        guard let status = SCFrameStatus(rawValue: statusRawValue) else   { return nil }
        if status != .complete {
            return nil
        }
        ...
        guard let contentRectDict = attachments[.contentRect],
              ...
              let dirtyRectsDict = attachments[.dirtyRects] as? Array<CFDictionary>,
        ...
        let frame = CapturedFrame(pixelBuffer: pixelBuffer,
                                  ...
                                  dirtyRects: dirtyRectsDict.map { CGRect(dictionaryRepresentation: $0) })
        return frame
    }
